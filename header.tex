...
\SetEnumitemKey{steps}{
  label=\underline{Step~\arabic*:},
  ref={\arabic*},
  align=left,
  leftmargin=\parindent
}
...
\newcommand{\refstar}{($\star$)}

\newcommand{\tagdagger}{\tag{$\dagger$}}
\newcommand{\refdagger}{($\dagger$)}

\newcommand{\tagddagger}{\tag{$\ddagger$}}
\newcommand{\refddagger}{($\ddagger$)}
...
\newenvironment{claim}{
  \let\qedsymbol\relax
  \Claim\item[\textbf{Claim :}]
}{
  \endClaim
  \let\qedsymbol\originalqedsymbol
}

\newlist{nClaim}{description}{1}
\setlist[nClaim]{
  labelindent=2em
}
\newcounter{nclaim}[nthm]
\renewcommand\thenclaim{\arabic{nclaim}}
\let\originalqedsymbol\qedsymbol
\newenvironment{nclaim}{
  \let\qedsymbol\relax
  \refstepcounter{nclaim}
  \nClaim\item[\textbf{Claim \thenclaim:}]
}{
  \endnClaim
  \let\qedsymbol\originalqedsymbol
}
...
\newcommand{\dddvec}[1]{\dddot{\vec{#1}}}
\newcommand{\ddddvec}[1]{\ddddot{\vec{#1}}}
...
\DeclareMathOperator{\gact}{\curvearrowright}
\DeclareMathOperator{\normalin}{\triangleleft}
\DeclareMathOperator{\hasnormal}{\triangleright}
...
% Composities
\newcommand{\spnset}[1]{\spn \crbkt{#1}}
\newcommand{\dimset}[1]{\dim \crbkt{#1}}
