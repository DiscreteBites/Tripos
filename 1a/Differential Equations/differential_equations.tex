...
It follows that
\begin{align*}
    y'' \eval{-\epsilon}{\epsilon} + p(x)y' \eval{-\epsilon}{\epsilon} + q(x)y \eval{-\epsilon}{\epsilon} = 1 \\
    \lim_{\epsilon \rightarrow 0} \sqbkt{y'' \eval{-\epsilon}{\epsilon} + p(0)y' \eval{-\epsilon}{\epsilon} + q(0)y \eval{-\epsilon}{\epsilon}} = 1 \tag{$\star$}
\end{align*}
...
\[
    \lim_{\epsilon \rightarrow 0} y' \eval{-\epsilon}{\epsilon} = 0 \text{ and } \lim_{\epsilon \rightarrow 0} y \eval{-\epsilon}{\epsilon} = 0
\]
...
\begin{align*}
    f(\vec x_0 + s \uvec s) &= f(\vec x_0) + s \diff fs \eval{\vec x_0}{} + \frac{1}{2} s^2 \diff[2] fs \eval{\vec x_0}{} + \cdots \\
    &= f(\vec x_0) + s \uvec s \cdot \nabla f \eval{\vec x_0}{} + \frac{1}{2} s^2 \uvec s \cdot \nabla(\uvec s \cdot \nabla) \eval{\vec x_0}{} + \cdots
\end{align*}
...
\begin{defi}[Multivariate Taylor Series]
    \begin{align*}
        f(x_0 + \delta x, y_0 + \delta y) = f(x_0, &y_0) + \bkt{\delta x \diffp fx + \delta y \diffp fy}\eval{x_0, y_0}{} \\
        &+ \frac{1}{2} \sqbkt{(\delta x)^2 \diffp[2] fx + 2\delta x \delta y \diffp{f}{{x}{y}} + (\delta y)^2 \diffp[2] fy}\eval{x_0, y_0}{} \\
        &+ \cdots
    \end{align*}
    In the coordinate free form we have
    \[
        f(\vec x_0 + \delta \vec x) = f(\vec x_0) + \delta \vec x \cdot (\nabla f) \eval{x_0}{} + \frac{1}{2} \delta \vec x \dunder{H} \delta \vec x^T + \cdots 
    \]
\end{defi}

%%%%%%%%%%%%%%%%%%%%%%
% Lecture 21: 20/11/2023
%%%%%%%%%%%%%%%%%%%%%%
